-- StackUp 0024: игроки с нулевым стеком пропускают раздачу.
--
-- Проблема: если у игрока current_stack = 0 (проиграл всё в предыдущей
-- раздаче), _begin_hand всё равно включал его в игру с hand_state='active'.
-- Ему назначался ход, и в ActionBar появлялся Колл/Пас, хотя колить ему
-- нечем, а значит это бессмысленно и вводит в заблуждение.
--
-- Решение в базе:
--   * В начале _begin_hand: нулевые стеки сразу получают hand_state='folded'
--     (+ has_acted=true чтобы не блокировать ход круга).
--   * Подсчёт участников раздачи (v_n) считает только игроков с chips > 0.
--   * Запросы выбора SB/BB/первого хода фильтруют по hand_state='active',
--     что автоматически пропускает "folded"-нулевиков.
--
-- Решение во фронте:
--   * Когда current_stack = 0 и игра между раздачами — показываем баннер
--     "Ваш стек пустой, попросите организатора сделать ребай" вместо ничего.
--   * ActionBar уже защищён условием me.hand_state === 'active' + это его ход,
--     так что кнопки у нулевика никогда не появятся.
...
begin
  ...
  -- Игроки с нулевым стеком пропускают раздачу (нужен ребай) — сразу
  -- помечаем их как folded, чтобы не назначать им ход и не давать действий.
  update players set round_bet = 0,
    hand_state = case when current_stack > 0 then 'active' else 'folded' end,
    has_acted  = case when current_stack > 0 then false   else true end,
    hand_contributed = 0, last_action = null
  where room_id = p_room_id and status = 'active';

  select count(*) into v_n from players where room_id = p_room_id and status = 'active' and current_stack > 0;
  if v_n < 2 then
  ...
  if v_n = 2 then
    ...
    where room_id = p_room_id and status = 'active' and hand_state = 'active' and id <> v_room.dealer_player_id
    limit 1;
    v_first := v_sb;
  else
    ...
    where room_id = p_room_id and status = 'active' and hand_state = 'active' and seat_index > coalesce(v_dealer_seat, -1)
    order by seat_index asc limit 1;
    if not found then
      select * into v_sb from players
      where room_id = p_room_id and status = 'active' and hand_state = 'active' order by seat_index asc limit 1;
    end if;

    select * into v_bb from players
    where room_id = p_room_id and status = 'active' and hand_state = 'active' and seat_index > v_sb.seat_index
    order by seat_index asc limit 1;
    if not found then
      select * into v_bb from players
      where room_id = p_room_id and status = 'active' and hand_state = 'active' order by seat_index asc limit 1;
    ...
    where room_id = p_room_id and status = 'active' and hand_state = 'active' and seat_index > v_bb.seat_index
    order by seat_index asc limit 1;
    if not found then
      select * into v_first from players
      where room_id = p_room_id and status = 'active' and hand_state = 'active' order by seat_index asc limit 1;
  ...
  if v_active_count <= 1 then
    ...
    update rooms set turn_player_id = v_first.id, turn_deadline = case when v_room.turn_timer_seconds is not null then now() + (v_room.turn_timer_seconds || ' seconds')::interval else null end, hand_stage = 'betting'
    where id = p_room_id;
  end if;
end;
$$;
...
begin
  ...
  if v_needs_action = 0 then
    ...
    update rooms set turn_player_id = v_next.id, turn_deadline = case when v_room.turn_timer_seconds is not null then now() + (v_room.turn_timer_seconds || ' seconds')::interval else null end
    where id = p_room_id;
  end if;
...
grant execute on function _begin_hand(uuid) to anon;
