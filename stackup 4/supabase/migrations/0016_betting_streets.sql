-- StackUp: реальные улицы торгов (Pre-Flop/Flop/Turn/River/Showdown)
-- вместо одного круга торгов на всю раздачу.
--
-- Раньше как только круг торгов закрывался (все уровняли ставки), раздача
-- сразу шла к выбору победителя — то есть после префлопа сразу шоудаун,
-- без флопа/тёрна/ривера. Это не соответствует Texas Hold'em.
--
-- Карты по-прежнему полностью физические — приложение их не видит и не
-- трогает. Но теперь оно отслеживает, на какой улице идёт раздача, и
-- после закрытия круга торгов (кроме ривера) ждёт явного нажатия
-- организатором «Следующая улица», прежде чем открыть новый круг —
-- организатору всё равно нужно физически выложить карту(ы) на стол
-- между улицами, так что кнопка — это и есть тот момент.
--
-- Важное следствие: round_bet и current_bet раньше считались за ВСЮ
-- раздачу, теперь они считаются ТОЛЬКО за текущую улицу (обнуляются
-- при переходе на новую улицу — банк при этом НЕ обнуляется, деньги
-- уже внутри него). Для сайд-потов и возврата банка при незавершённой
-- раздаче (Finish Game) нужна была сумма за ВСЮ раздачу — для этого
-- добавлено отдельное поле players.hand_contributed, которое копит
-- вклад игрока за все улицы текущей раздачи и обнуляется только в
-- начале новой раздачи.
...
-- ============================================================
-- _begin_hand: то же самое, что в 0014, плюс сброс street в 'preflop' и
-- hand_contributed в 0; постановка блайндов пишет и в round_bet
-- (улица), и в hand_contributed (вся раздача) — это начало и того, и
...
begin
  ...
  update players set round_bet = 0, hand_state = 'active', has_acted = false, hand_contributed = 0
...
-- ============================================================
-- _apply_action: та же логика действий, что в 0014, плюс:
-- (а) каждое движение фишек пишется и в round_bet (эта улица), и в
--     hand_contributed (вся раздача);
-- (б) закрытие круга торгов больше не ведёт сразу к awaiting_winner —
--     если улица не последняя (river), круг просто закрывается
--     (turn_player_id = null), и raздача ждёт next_stage.
-- ============================================================
...
-- ============================================================
-- next_stage: организатор переводит раздачу на следующую улицу после
-- того, как круг торгов закрылся (turn_player_id уже null) и физически
-- выложил карту(ы) на стол. Деньги не двигаются — это чисто переход
-- стадии плюс открытие нового круга торгов (current_bet/round_bet/
-- has_acted обнуляются для новой улицы, банк не трогаем).
-- ============================================================
...
begin
  ...
  update players set round_bet = 0, has_acted = false
...
-- ============================================================
-- _compute_side_pots: тот же алгоритм, что в 0009, но по hand_contributed
-- (вклад за всю раздачу), а не round_bet (который теперь только за
-- текущую улицу и для сайд-потов больше не подходит).
-- ============================================================
...
-- ============================================================
-- finish_game: тот же refund-вместо-ошибки подход, что в 0014, но
-- возвращает hand_contributed (вклад за ВСЮ раздачу, все улицы), а не
-- round_bet (только текущая улица) — иначе при отмене раздачи на
-- Turn/River потерялись бы деньги, внесённые на Preflop/Flop.
-- ============================================================
