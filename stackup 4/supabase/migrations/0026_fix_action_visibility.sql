-- StackUp 0026: фикс отображения действий игроков у других участников
--
-- Проблема: когда действие игрока (call/raise/fold) было последним в стрите,
-- next_stage сразу обнулял last_action = null в той же транзакции.
-- Supabase Realtime шлёт оба UPDATE одной строки как отдельные сообщения,
-- но клиент мог получить только второй (null), пропустив bubble с действием.
--
-- Фикс: убрать last_action = null из next_stage.
-- Теперь last_action сбрасывается ТОЛЬКО в _begin_hand (начало новой раздачи).
-- Побочный эффект: игроки видят последнее действие соперника до следующего
-- его хода — это лучше, не хуже (бubble всё равно 1800мс).
...
begin
  ...
  -- last_action НЕ сбрасывается здесь: сброс только в _begin_hand.
  -- Причина: сброс в next_stage создаёт race condition в Realtime —
  -- два UPDATE одной строки в одной транзакции, клиенты могут получить
  -- только последний (null), пропустив bubble с действием игрока.
