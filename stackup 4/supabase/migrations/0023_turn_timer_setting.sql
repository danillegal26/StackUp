-- StackUp 0023: настройка таймера хода (организатор может задать
-- длительность или отключить таймер вообще).
--
-- Добавляет колонку rooms.turn_timer_seconds (int, nullable):
--   - null  = таймер отключён (ход без ограничения по времени)
--   - число = кол-во секунд (10-120), по умолчанию 25 как раньше
-- По умолчанию для существующих комнат — 25 (backward-compatible).
--
-- Функции _begin_hand, _apply_action, next_stage перезаписаны, чтобы
-- учитывать turn_timer_seconds при установке turn_deadline:
--   - если null → turn_deadline = null (check_timeout никогда не сработает)
--   - иначе     → turn_deadline = now() + N seconds
--
-- Новая RPC set_turn_timer(room_id, host_token, seconds?) позволяет
-- организатору менять таймер в любой момент (вступает в силу со
-- следующего хода).
--
-- Также добавляет turn_timer_seconds в CREATE TABLE для новых комнат —
-- это значит его надо передать в create_room, а стартовое значение
-- выбирается при создании стола. Для уже существующих комнат ставим 25.

alter table rooms add column if not exists turn_timer_seconds integer
  check (turn_timer_seconds is null or (turn_timer_seconds >= 10 and turn_timer_seconds <= 120))
  default 25;

-- Убедимся, что у существующих комнат (где колонка только что появилась)
-- стоит 25 — DEFAULT не применяется к уже существующим строкам.
update rooms set turn_timer_seconds = 25 where turn_timer_seconds is null;
...
create or replace function set_turn_timer(
  p_room_id        uuid,
  p_host_token     uuid,
...
begin
  ...
  if p_seconds is not null and (p_seconds < 10 or p_seconds > 120) then
    raise exception 'Timer must be between 10 and 120 seconds, or null to disable';
  end if;
  update rooms set turn_timer_seconds = p_seconds where id = p_room_id;
end;
$$;

grant execute on function set_turn_timer(uuid, uuid, integer) to anon;
