...
export function RebuyModal({ player, defaultAmount, onClose, onSubmit }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Не удалось зафиксировать докупку');
      setSubmitting(false);
    }
  }
  ...
  return (
    <Modal title={`Rebuy / Add-on: ${player.name}`} onClose={onClose}>
      <div className="mb-4 flex gap-2">
        <Button
          variant={type === 'rebuy' ? 'primary' : 'secondary'}
          className="flex-1"
          onClick={() => setType('rebuy')}
        >
          Rebuy
        </Button>
        <Button
          variant={type === 'addon' ? 'primary' : 'secondary'}
          className="flex-1"
      ...
      <label className="mb-1 block text-xs text-muted">Сумма докупки</label>
      <Input
        type="number"
        inputMode="numeric"
        value={amount}
      ...
      <Button className="w-full" disabled={submitting} onClick={handleSubmit}>
        {submitting ? 'Добавляем…' : 'Зафиксировать'}
      </Button>
    </Modal>
  );
}
