...
const MAX_POT_LAYERS = 4; // банк — самое тесное место на столе (под ним ещё текст и слоты карт), здесь сдержаннее, чем у личных стопок
...
function FlyingChip({
  ...
  return (
    <div
      className="pointer-events-none absolute z-10 -translate-x-1/2 -translate-y-1/2 drop-shadow-[0_0_6px_rgba(245,158,11,0.55)] transition-all duration-700 ease-out"
...
export function PokerTable({
  ...
  // Банк масштабируется относительно ВСЕХ фишек в игре (банк + стеки всех
  // игроков, эта сумма постоянна), а не абсолютного числа — так стопка в
  // центре растёт заметно при любых блайндах/стартовых стеках.
  const totalChips = pot + sorted.reduce((sum, p) => sum + p.current_stack, 0);
  const potLayers =
    pot <= 0 ? 0 : Math.max(1, Math.min(MAX_POT_LAYERS, Math.round((pot / Math.max(1, totalChips)) * MAX_POT_LAYERS)));
  ...
  useEffect(() => {
    ...
    sorted.forEach((p) => {
      ...
      if (p.last_action && p.last_action !== prevAction && ACTION_LABELS[p.last_action]) {
        spawned.push({ key: `${p.id}-${p.last_action}-${Date.now()}`, playerId: p.id, label: ACTION_LABELS[p.last_action] });
      }
  ...
  useEffect(() => {
    ...
    sorted.forEach((p, i) => {
      ...
      targets.push({ key: `win-${p.id}-${winAnnouncement.key}`, to: seatPos(i, n) });
    });
  ...
  return (
    <div className="relative mx-auto aspect-[4/5] w-full max-w-sm select-none py-2">
      {/* Рейл стола (тёмная кайма с тонкой латунной окантовкой) — под зелёным сукном, чуть больше его по размеру, выглядывает по краю */}
      <div
        className="absolute left-1/2 top-1/2 h-[63%] w-[85%] -translate-x-1/2 -translate-y-1/2 rounded-[50%] bg-coal shadow-[0_10px_28px_-6px_rgba(0,0,0,0.55)]"
        aria-hidden="true"
      />
      <div
        className="absolute left-1/2 top-1/2 h-[60.5%] w-[82.5%] -translate-x-1/2 -translate-y-1/2 rounded-[50%] border border-brass/30"
        aria-hidden="true"
      />
      ...
      <div
        className="absolute left-1/2 top-1/2 h-[58%] w-[80%] -translate-x-1/2 -translate-y-1/2 rounded-[50%] border-2 border-table-glow/40 shadow-[0_0_36px_-6px_rgba(25,169,116,0.45)]"
      ...
      <div className="absolute left-1/2 top-1/2 z-[5] -translate-x-1/2 -translate-y-1/2 text-center">
        ...
        <div key={potPulse} className={`flex justify-center ${pot > 0 ? 'animate-chip-pulse' : ''}`}>
          <ChipStackIcon size={pot > 0 ? 30 : 22} count={potLayers} />
        </div>
          ...
          <Fragment key={p.id}>
            {!isFolded && pileLayers > 0 && (
              <div
                ...
                aria-hidden="true"
              >
                <ChipStackIcon size={18} count={pileLayers} />
              </div>
            )}
            <div
            ...
            {activeBubble && (
              <span
                ...
                className="absolute -top-1.5 left-1/2 z-[6] -translate-x-1/2 -translate-y-full whitespace-nowrap rounded-full border border-hairline/15 bg-coal/90 px-2 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-wide text-paper shadow-sm"
              >
                {activeBubble.label}
              </span>
            )}
