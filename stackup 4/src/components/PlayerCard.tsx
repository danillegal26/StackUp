...
export function PlayerCard({
  player, index, highlight, onAdjust, onRebuy, onRemove }: Props) {
  const t = useT();
  ...
  return (
    ...
    >
      <ChipAvatar name={player.name} index={index} avatar={getAvatarImage(player.avatar_id)} />
      <div className="min-w-0 flex-1">
        <p className="truncate font-medium text-ivory">
          {player.name}
          {highlight ? ` (${t.actingYou})` : ''}
          {player.is_host ? ' · организатор' : ''}
          {left ? ' · ушёл' : ''}
        </p>
