...
export function WinBanner({
  data }: { data: WinBannerData }) {
  const t = useT();
  const text = data.names.length === 1 ? data.names[0] : data.names.join(t.winsBankAnd);
  ...
  return (
    ...
    >
      <div className="animate-win-pop rounded-2xl border border-brass/50 bg-felt-deep/95 px-6 py-4 text-center shadow-[0_0_40px_-4px_rgba(245,158,11,0.5)]">
        <p className="font-display text-lg font-semibold text-brass-light">{text} {t.winsBank}</p>
