...
const TURN_SECONDS_DEFAULT = 25; // запасное значение когда room.turn_timer_seconds ещё не пришёл
...
export function ActionBar({
  room, me, secondsLeft, onAction }: Props) {
  const t = useT();
  ...
  const totalSeconds = room.turn_timer_seconds ?? TURN_SECONDS_DEFAULT;
  ...
  function run(action: PlayerActionType, amount?: number) {
    if (busy) return;
    setBusy(true);
    setError(null);
    // Не ждём ответа сервера — Realtime обновит UI автоматически, как только
    // action применится. Кнопки разблокируются через 600мс (достаточно чтобы
    // предотвратить двойной клик, но не морозит UI на всё время round-trip).
    onAction(action, amount).catch((err) => {
      setError(translateError(err, t.errAction));
      setBusy(false);
    });
        ...
        <div className="flex items-center gap-2">
          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-hairline/10">
            <div
              ...
              style={{ width: `${clamp((secondsLeft / totalSeconds) * 100, 0, 100)}%` }}
            />
          </div>
      ...
      <div className="space-y-3 md:flex md:items-start md:gap-4 md:space-y-0">
        <div className="grid grid-cols-3 gap-2 md:w-auto md:shrink-0">
          ...
          <Button variant="secondary" size="sm" disabled={busy} onClick={() => run(canCheck ? 'check' : 'call')}>
            {canCheck ? t.check : t.call(formatChips(callAmount))}
          </Button>
          <Button
            size="sm"
          ...
          >
            {room.current_bet > 0 ? t.raise(formatChips(raiseAmount)) : t.bet(formatChips(raiseAmount))}
          </Button>
        </div>
          ...
          <div className="space-y-3 md:min-w-0 md:flex-1">
            <div className="flex items-center justify-between px-1">
              <span className="font-mono text-[10px] uppercase tracking-widest text-muted">
                {room.current_bet > 0 ? t.raiseAmount : t.betAmount}
              </span>
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                aria-label={room.current_bet > 0 ? t.raiseAmount : t.betAmount}
              />
              <button
                type="button"
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-hairline/15 text-lg text-ivory disabled:opacity-30"
