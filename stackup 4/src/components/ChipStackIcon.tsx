...
export function ChipStackIcon({ size = 28, count = 3, colorIndex }: { size?: number; count?: number; colorIndex?: number }) {
  ...
  return (
    <span className="relative inline-block" style={{ width: size, height: totalHeight }} aria-hidden="true">
      {Array.from({ length: n }).map((_, idx) => (
        ...
        >
          <ChipFace size={size} palette={PALETTE[(colorIndex ?? idx) % PALETTE.length]} />
        </span>
      ))}
    </span>
  );
