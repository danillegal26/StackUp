...
export function WinnerSelectModal({
  players, onClose, onSubmit }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t.errAwardPot);
      setSubmitting(false);
    }
  }
  ...
  if (pots.length === 0) {
    return (
      <Modal title={t.selectWinner} onClose={onClose}>
  ...
  return (
    <Modal title={t.selectWinner} onClose={onClose}>
            ...
            <div key={potIdx}>
              <div className="mb-2 text-center">
                <p className="font-mono text-[10px] uppercase tracking-widest text-muted">
                  {pots.length > 1 ? (potIdx === 0 ? t.mainPot : `${t.sidePot} ${potIdx}`) : t.sidePot}
                </p>
                <p className="font-mono text-2xl font-semibold tabular-nums text-brass-light">
                  {formatChips(pot.amount)}
                </p>
                {selected.size > 1 && (
                  <p className="mt-1 text-xs text-muted">÷ {selected.size}</p>
                )}
              </div>
      ...
      <Button className="mt-4 w-full" onClick={handleSubmit} disabled={submitting || !allPotsHaveWinner}>
        {submitting ? t.distributing : t.confirmHand}
      </Button>
    </Modal>
  );
}
