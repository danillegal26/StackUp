...
import type { CreatedRoom, HandHistoryEntry, Player, PlayerResult, Room, Transaction, TransactionType } from '../types';
...
export async function joinRoom(
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) throw error;
  const row = data as { player_id: string; player_token: string };
...
export async function awardPot(roomId: string, hostToken: string, winnerIds: string[]): Promise<void> {
  const { error } = await supabase.rpc('award_pot', {
    p_room_id: roomId,
    p_host_token: hostToken,
...
export async function awardPots(roomId: string, hostToken: string, potWinners: string[][]): Promise<void> {
  const { error } = await supabase.rpc('award_pots', {
    p_room_id: roomId,
    p_host_token: hostToken,
    p_pot_winners: potWinners,
  });
  if (error) throw error;
}

export async function leaveTable(roomId: string, playerId: string, playerToken: string): Promise<void> {
...
export async function getHandHistory(roomId: string): Promise<HandHistoryEntry[]> {
  const { data, error } = await supabase
    .from('hand_history')
    .select('*')
    .eq('room_id', roomId)
...
export async function checkTimeout(roomId: string): Promise<void> {
  const { error } = await supabase.rpc('check_timeout', { p_room_id: roomId });
  if (error) throw error;
}

export async function nextStage(roomId: string, hostToken: string): Promise<Room> {
  const { data, error } = await supabase
    .rpc('next_stage', { p_room_id: roomId, p_host_token: hostToken })
    .single();
  if (error) throw error;
  return data as Room;
}
