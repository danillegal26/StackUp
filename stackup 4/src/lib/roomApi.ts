...
export async function hostJoinRoom(roomId: string, hostToken: string, name: string): Promise<Player> {
  const { data, error } = await supabase
    .rpc('host_join_room', { p_room_id: roomId, p_host_token: hostToken, p_name: name })
    .single();
  if (error) throw error;
  return data as Player;
}
