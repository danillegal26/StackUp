      ...
      {room.status === 'active' && (
        <>
          <section className="mb-4">
            ...
            {room.hand_stage === 'awaiting_winner' && room.pot > 0 && (
              <div className="mb-4 space-y-3">
                <Banner kind="info">Торги завершены — определите победителя по картам и нажмите кнопку ниже.</Banner>
          ...
          <Button
            ...
            disabled={finishing || players.length === 0 || potUnresolved}
          >
            {finishing ? 'Завершаем…' : 'Завершить игру'}
          </Button>
          {potUnresolved && (
            <p className="mt-2 text-center text-xs text-muted">
              Сначала завершите текущую раздачу и распределите банк {formatChips(room.pot)}
            </p>
          )}
        </>
      )}
