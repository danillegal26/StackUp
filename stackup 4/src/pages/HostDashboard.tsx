...
export default function HostDashboard() {
  ...
  // ⚠️ ВСЕ хуки — до любых условных return, иначе React крашится
  // (Rules of Hooks: одинаковое число хуков на каждом рендере)
  ...
  const potUnresolved = (room?.hand_stage ?? 'idle') !== 'idle' && (room?.pot ?? 0) > 0;
            ...
            <div className="mb-6">
              <ActionBar
                ...
                onAction={(action: PlayerActionType, amount?: number) => {
                  ...
                  return playerAction(roomId, me.id, mySession.playerToken, action, amount).then(() => {});
                }}
              />
            </div>
          )}
