...
export default function PlayerView() {
  ...
  async function handleLeave() {
    ...
    try {
      ...
      setLeaveError(err instanceof Error ? err.message : 'Не удалось покинуть стол');
    } finally {
      setLeaving(false);
    }
  }
  ...
  return (
    <div className="mx-auto min-h-screen max-w-md px-4 py-8">
      ...
      {room.status === 'active' && room.hand_stage === 'awaiting_winner' && room.pot > 0 && (
        <div className="mb-6">
          <Banner kind="info">
            Раздача завершена, банк {formatChips(room.pot)} — организатор сейчас выберет победителя.
          </Banner>
        </div>
      )}
          ...
          <div className="mb-6">
            <ActionBar
              ...
              onAction={async (action: PlayerActionType, amount?: number) =>
                void (await playerAction(roomId, me.id, session.playerToken, action, amount))
              }
            />
          </div>
        )}
